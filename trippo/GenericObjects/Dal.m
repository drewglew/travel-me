    ...
    if([filemgr fileExistsAtPath:_databasePath] ==  NO) {
        ...
        if (sqlite3_open(dbpath, &_DB) == SQLITE_OK) {
            ...
            */
            ...
            if (![self checkColumnExists:@"filename" :@"imageactivity"] ) {
                char *errorMessage;
                 const char *sql_statement = "CREATE TABLE imageactivity (filename TEXT, activitykey TEXT, keyimage INTEGER, description TEXT, state INTEGER, PRIMARY KEY (filename, activitykey, state), FOREIGN KEY(activitykey,state) REFERENCES activity(key,state))";
            
                if(sqlite3_exec(_DB, sql_statement, NULL, NULL, &errorMessage) != SQLITE_OK) {
                    NSLog(@"failed to create imageactivity table");
                }
            }
...
-(bool)CreateDb {
    bool retVal=true;
        ...
        sql_statement = "CREATE TABLE imageactivity (filename TEXT, activitykey TEXT, keyimage INTEGER, description TEXT, state INTEGER, PRIMARY KEY (filename, activitykey, state), FOREIGN KEY(activitykey,state) REFERENCES activity(key,state))";
...
-(bool) UpdateActivityItem :(ActivityNSO*) Activity {
    ...
    [self InsertActivityImages :Activity];
    [self UpdateActivityImages :Activity];
...
/*
 ...
 remarks:           TODO add new way of inserting parms
 */
-(bool)UpdateActivityImages :(ActivityNSO*) Activity {
    /* Images table */
    bool retVal = true;
    
    for (ImageNSO *imageitem in Activity.Images) {
        if (imageitem.UpdateImage) {
            
            NSString *insertSQL = [NSString stringWithFormat:@"UPDATE imageactivity set keyimage=%d, description='%@' WHERE filename='%@'", imageitem.KeyImage, @"test", imageitem.ImageFileReference];
            sqlite3_stmt *statement;
            const char *update_statement = [insertSQL UTF8String];
            sqlite3_prepare_v2(_DB, update_statement, -1, &statement, NULL);
            if (sqlite3_step(statement) != SQLITE_DONE) {
                NSLog(@"Failed to update record inside imageactivity table");
                retVal = false;
            } else {
                NSLog(@"Updated imageactivity record inside table!");
            }
            sqlite3_finalize(statement);
        }
    }
...
-(NSMutableArray*) GetProjectContent :(NSString*) RequiredKey {
    ...
    NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:@"startdt" ascending:NO];
    
    [projectdata sortUsingDescriptors:[NSArray arrayWithObject:sort]];
...
/*
 ...
 remarks:
 */
-(bool)InsertActivityImages :(ActivityNSO*) Activity {
    /* Images table */
    bool retVal = true;
    ...
    for (ImageNSO *imageitem in Activity.Images) {
        if (imageitem.NewImage) {
            ...
            NSString *insertSQL = [NSString stringWithFormat:@"INSERT INTO imageactivity (filename, activitykey, keyimage, description, state) VALUES ('%@','%@', %d ,'%@', %@)", imageitem.ImageFileReference, Activity.key, imageitem.KeyImage, @"test", imageitem.State];
            sqlite3_stmt *statement;
            const char *insert_statement = [insertSQL UTF8String];
            sqlite3_prepare_v2(_DB, insert_statement, -1, &statement, NULL);
            if (sqlite3_step(statement) != SQLITE_DONE) {
                NSLog(@"Failed to insert new record inside imageactivity table");
                retVal = false;
            } else {
                NSLog(@"Inserted new imageactivity record inside table!");
            }
            sqlite3_finalize(statement);
        }
    }
...
-(NSMutableArray *)GetImagesForSelectedActivity :(NSString *) RequiredKey :(NSNumber*) RequiredState {
    ...
    NSString *selectSQL = [NSString stringWithFormat:@"SELECT filename, keyimage, description FROM imageactivity WHERE activitykey='%@' and state=%@", RequiredKey, RequiredState];
    ...
    if (sqlite3_prepare_v2(_DB, select_statement, -1, &statement, NULL) == SQLITE_OK)
    {
        while (sqlite3_step(statement) == SQLITE_ROW)
        {
            ImageNSO *imgitem = [[ImageNSO alloc] init];
            imgitem.ImageFileReference = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 0)];
            imgitem.KeyImage = sqlite3_column_int(statement, 1);
            imgitem.NewImage = false;
            [ImagesDataSet addObject:imgitem];
...
/*
 created date:      02/05/2018
 last modified:     20/08/2018
 remarks:           WIP - need to manage what files to delete when removing a single Activity against one which
                    removes the whole projects set of Activities.  Maybe worth placing the activities inside
                    Project reference folder?   Images/Projects/key/Activities/key/file
 */
-(bool) DeleteActivity :(ActivityNSO*) Activity :(NSString*) ProjectKey {
    ...
    if (Activity==nil) {
        ...
        [self DeleteActivityAttachments :nil :ProjectKey];
        
        dataPath = [imagesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"/Images/Projects/%@/Activities", ProjectKey]];
        NSError *error = nil;
        for (NSString *file in [fm contentsOfDirectoryAtPath:dataPath error:&error]) {
        ...
        [self DeleteActivityAttachments :Activity :nil];
        
        
        dataPath = [imagesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"/Images/Projects/%@/Activities/%@", ProjectKey, Activity.key]];
...
/*
 ...
 remarks:
 */
-(bool) DeleteActivityAttachments :(ActivityNSO *) Activity  :(NSString *) ProjectKey {
    sqlite3_stmt *statement;
    bool retVal = true;
    NSString *deleteSQL;
    
    if (ProjectKey!=nil) {
        deleteSQL = [NSString stringWithFormat:@"DELETE FROM imageactivity WHERE activitykey in (SELECT key FROM activity WHERE projectkey='%@')", ProjectKey];
    } else {
    
        deleteSQL = [NSString stringWithFormat:@"DELETE FROM imageactivity WHERE activitykey = '%@' and state=%@", Activity.key, Activity.activitystate];
    }
    const char *deleteStatement = [deleteSQL UTF8String];
    sqlite3_prepare_v2(_DB, deleteStatement, -1, &statement, NULL);
    
    if (sqlite3_step(statement) != SQLITE_DONE) {
        NSLog(@"Failed to delete items from imageactivity");
        retVal = false;
    } else {
        NSLog(@"Successfuly deleted items from imageactivity");
    }
    sqlite3_finalize(statement);
    return true;
}
...
-(bool) DeleteActivityImage :(ImageNSO*) ImageItem {
    ...
    sqlite3_stmt *statement;
    NSString *deleteSQL = [NSString stringWithFormat:@"DELETE FROM imageactivity WHERE filename = '%@'",ImageItem.ImageFileReference];
    const char *deleteStatement = [deleteSQL UTF8String];
    sqlite3_prepare_v2(_DB, deleteStatement, -1, &statement, NULL);
